...
export const getBlocks = async (req: Request, res: Response) => {
  try {
    ...
    const [blocksResultSet, countResultSet] = await Promise.all([
      chClient.query({
        query: `
                        SELECT
                            f_el_block_number,
...
export const getBlockById = async (req: Request, res: Response) => {
  try {
    ...
    const blockResultSet = await chClient.query({
      query: `
                    SELECT
                        f_el_block_number,
                        f_el_block_hash,
                        ...
                        f_payload_size_bytes
                    FROM t_block_metrics
                    WHERE f_el_block_number = {id: Int64}
                    LIMIT 1
                `,
      query_params: { id },
      format: "JSONEachRow",
...
export const getTransactionsByBlock = async (req: Request, res: Response) => {
  try {
    ...
    const transactionsResultSet = await chClient.query({
      query: `
                    SELECT
                        f_tx_type,
                        f_value,
