...
export const getEpochsStatistics = async (req: Request, res: Response) => {
  try {
        ...
        clickhouseClient
          .query({
            query: `
                        ...
                        OFFSET {offset: Int64}
                    `,
            query_params: { limit: Number(limit) + 1, offset },
            format: "JSONEachRow",
          })
          .catch((err: any) => {
    ...
    epochsStatsResult.forEach((epoch: any) => {
      const aux = blocksStatsResult.find(
        (blocks: any) => Number(blocks.epoch) === Number(epoch.f_epoch),
      );
      arrayEpochs.push({
        ...epoch,
        ...
        clickhouseClient.query({
          query: `
                        ...
                        FROM
                            t_proposer_duties
                        WHERE
                            CAST((f_proposer_slot / 32) AS UInt64) = {id: Int64} AND f_proposed = 1
                    `,
          query_params: { id },
          format: "JSONEachRow",
        }),
        clickhouseClient.query({
          query: `
                        ...
                        FROM
                            t_withdrawals
                        WHERE
                            CAST((f_slot / 32) AS UInt64) = {id: Int64}
                    `,
          query_params: { id },
          format: "JSONEachRow",
        }),
...
export const getSlotsByEpoch = async (req: Request, res: Response) => {
  try {
    ...
    const withdrawalsCounts: any[] = withdrawalsResult.reduce(
      (acc, withdrawal) => {
    ...
    const slotCount: any[] = Object.entries(withdrawalsCounts).map(
      ([slot, data]) => ({ slot: parseInt(slot), count: data.count }),
    );
    ...
    const slots = slotsEpochResult.map((slot: any) => ({
      ...
      withdrawals: withdrawalsResult
        ...
        .reduce(
          (acc: number, withdrawal: any) => acc + Number(withdrawal.f_amount),
          0,
        ),
      num_withdrawals:
        slotCount.find((result) => result.slot === slot.f_proposer_slot)
          ?.count || 0,
    }));
...
export const listenEpochNotification = async (req: Request, res: Response) => {
  try {
    ...
    const newEpochEstimatedTime =
      genesisTime + (nextEpoch + 2) * 12000 * 32 + 4000;

    await new Promise((resolve) =>
      setTimeout(resolve, newEpochEstimatedTime - Date.now()),
    );
