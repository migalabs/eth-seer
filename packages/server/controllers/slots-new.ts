...
export const getSlots = async (req: Request, res: Response) => {
  try {
    ...
    if (epoch) {
      ...
      params.push(
        { epoch: Number(epoch) * 32 },
        { epoch1: (Number(epoch) + 1) * 32 },
      );
    }

    if (lowerEpoch) {
      where.push(`(pd.f_proposer_slot >= {lowerEpoch: Int64})`);
      params.push({ lowerEpoch: Number(lowerEpoch) * 32 });
    }

    if (upperEpoch) {
      where.push(`(pd.f_proposer_slot < {upperEpoch: Int64})`);
      params.push({ upperEpoch: (Number(upperEpoch) + 1) * 32 });
    }

    if (validator) {
      where.push(`(pd.f_val_idx = {validator: Int64})`);
      params.push({ validator: Number(validator) });
    }
    ...
    if (entities && Array.isArray(entities) && entities.length > 0) {
      const entitiesArray = entities
        ...
        .filter((x) => x !== "");
      where.push(`(pk.f_pool_name IN ({entities: String}))`);
      params.push({ entities: entitiesArray.join(",") });
    }
    ...
    if (clients && Array.isArray(clients) && clients.length > 0) {
      const clientsArray = clients
        ...
        .filter((x) => x !== "");
      where.push(`(LOWER(scg.f_best_guess_single) IN ({clients: String}))`);
      params.push({ clients: clientsArray.join(",") });
    }
